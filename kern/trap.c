...
trap_init_percpu(void)
{
	...
	wrmsr(IA32_SYSENTER_ESP, KSTACKTOP - cpunum() * (KSTKSIZE + KSTKGAP), 0);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall) {
		sp = tf->tf_esp;
		if ((sp >= UXSTACKTOP) || (sp < (UXSTACKTOP - PGSIZE)))
			sp = UXSTACKTOP;

		user_mem_assert(curenv,
				(void *) (sp - sizeof(struct UTrapframe) - 4),
